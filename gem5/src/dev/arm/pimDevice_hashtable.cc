...
#if (USE_PIM_DEVICE == PIM_DEVICE_HASH_TABLE)

#include "dev/arm/pimDevice_hashtable.hh"
...
void PimDevice::startBtreeTraverse(uint64_t ch)
{
	...
	/* For the item pointer */
	leafAddr[ch] = rootAddr[ch];
	btNodeCrossPage[ch] = isBtNodeCrossPage(rootAddr[ch]);

	/* For the key, we use the next channel for now */
	btNodeCrossPage[ch+1] = isKeyCrossPage(key[ch], ch);
	...
	transVirtAddrToPhysAddr(key[ch], ch+1);
}
...
// whether the address of key exceeds a page
bool PimDevice::isKeyCrossPage(unsigned long addr, uint64_t ch)
{
	return ((addr & ~(PAGE_MASK)) != ((addr + keySize[ch] - 1) & ~(PAGE_MASK)));
}
...
void PimDevice::pageWalkDone(uint64_t physAddr, uint64_t ch)
{
	...
	// Channel 1 is used to fetch key
	if (ch == 1) {
		if (!btNodeCrossPage[ch]) {
			...
			if (isKeyCrossPage(physAddr, 0)) {
				// The head of key that cross the page
				DPRINTF(PIM, "Fetch the Key, ch =%d\n", ch);
				...
				// The tail of key that cross the page
				DPRINTF(PIM, "Fetch the tail of key, ch =%d\n", ch);
				fetchSize = keySize[0] - btNodeCrossPageHeadSize[ch];			
...
void PimDevice::fetchBtNodeBack(uint64_t ch)
{
	...
	if (ch == 0)   // Skip the latency for keys
	...
	// Set the flag for node or key availability
	if (ch == 1)
		isKeyAvailable = true;
	else if (ch == 0)
		isNodeAvailable = true;

	if (isKeyAvailable && isNodeAvailable) {

		// Compare the keys
		if (memcmp(&btNodeDmaBuffer[1], btNodeDmaBuffer[0].data, keySize[0]) == 0) {
			startTrav[0] = false;
			doneTrav[0] = true;
			totalLatency[0] += curTick() - travRequestStart[0];
			DPRINTF(PIM, "Found matched hash node at %lx at ch %d\n", leafAddr[0], 0);
		} else {
			// Go to get the next node, first we translate the VA to PA
			// After that, the BT node will be fetched automatically
			leafAddr[0] = (uint64_t)btNodeDmaBuffer[0].h_next;

			btNodeCrossPage[0] = isBtNodeCrossPage(leafAddr[0]);
			transVirtAddrToPhysAddr(leafAddr[0], 0);
		}
	}
