...
// The implemetation that takes advantage of hardware indexing traversal and doing things
// in parallel instead of just polling
...
RC tpcc_txn_man::run_payment(tpcc_query * query) {
	...
	index_read_issue(index, key, wh_to_part(w_id));

	key = distKey(query->d_id, query->d_w_id);    //OPT: pre-calculate the next key

	item = index_read_result(index);
	index_read_issue(_wl->i_district, key, wh_to_part(w_id));  //OPT: issue next read
...
//	item = index_read(_wl->i_district, key, wh_to_part(w_id));
	item = index_read_result(_wl->i_district);
	index_read_issue(index, key, wh_to_part(w_id));  //OPT: issue next read
...
//	rc = index->index_read(key, item, wh_to_part(w_id));
	...
	key = custKey(c_id, d_id, w_id);    //OPT: pre-calculate the next key
	...
	index_read_issue(_wl->i_district, key, wh_to_part(w_id)); 
...
//	assert(rc == RCOK);
	...
	//OPT: issue next read
	if (ol_cnt > 0) {
		uint64_t ol_i_id = query->items[0].ol_i_id;
		index_read_issue(_wl->i_item, ol_i_id, 0);
	}
...
//	assert(rc == RCOK);
	...
	for (UInt32 ol_number = 0; ol_number < ol_cnt; ol_number++) {
		...
		+===========================================*/
		//key = ol_i_id;
		key = stockKey(ol_i_id, ol_supply_w_id);   //OPT: Pre-calculate next key

		//item = index_read(_wl->i_item, key, 0);
		item = index_read_result(_wl->i_item);
		assert(item != NULL);

		//OPT: Issue next index read
		index = _wl->i_stock;
		index_read_issue(index, key,  wh_to_part(ol_supply_w_id));
...
//		assert(rc == RCOK);
		...
		//OPT: issue next read
		if ((ol_number + 1) < ol_cnt) {
			uint64_t ol_i_next_id = query->items[ol_number + 1].ol_i_id;
			index_read_issue(_wl->i_item, ol_i_next_id, 0);
		}
