...
void txn_man::index_read_issue(INDEX * index, idx_key_t key, int part_id)
{
	//	uint64_t starttime = get_sys_clock();
	index->index_read_issue(key, part_id, get_thd_id());
	//INC_TMP_STATS(get_thd_id(), time_index, get_sys_clock() - starttime);
}

itemid_t * txn_man::index_read_result(INDEX * index)
{
	uint64_t starttime = get_sys_clock();
	itemid_t * item;
	index->index_read_result(item, get_thd_id());
	INC_TMP_STATS(get_thd_id(), time_index, get_sys_clock() - starttime);
	return item;
}
