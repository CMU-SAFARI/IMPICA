...
void Stats::print() {
	...
	for (uint64_t tid = 0; tid < g_thread_cnt; tid ++) {
		...
		printf("[tid=%ld] txn_cnt=%ld,abort_cnt=%ld"
			   ...
			   (double)_stats[tid]->run_time / BILLION,
			   (double)_stats[tid]->time_wait / BILLION,
			   (double)_stats[tid]->time_ts_alloc / BILLION,
			   ((double)_stats[tid]->time_man - (double)_stats[tid]->time_wait) / BILLION,
			   (double)_stats[tid]->time_index / BILLION,
			   (double)_stats[tid]->time_abort / BILLION,
			   (double)_stats[tid]->time_cleanup / BILLION,
			   (double)_stats[tid]->latency / BILLION,
			   (double)_stats[tid]->time_query / BILLION);
	}
	FILE * outf;
	if (output_file != NULL) {
